### SNIPPET
  # n = gets.split.map(&:to_i)
  # array = n.times.map { gets.split.map(&:to_i) }
  # [].all?(&:even?)
  # a = [*1..m].repeated_combination(n).to_a
  # [1,2,3,4,5].select { |num| num.even? }  # => [2, 4]
  # ["a","a","b"].group_by(&:itself).map{|k,v| [k, v.count]}.to_h #=> {"a"=>2, "b"=>1}
  # 切り捨て: .floor(2).to_f ,切り上げ： .ceil(2).to_f,四捨五入: round(2)
  # 3.upto(6) do |i|, 6.downto(3) do |i|
  # 公約数125.gcd(100)、公倍数125.lcm(100)
  # PI = Math::PI
  # 高さ = a * Math.sin(w / 180.0 * Math::PI), 底辺 = a * Math.cos(w / 180.0 * Math::PI)
  # def chmax(a, b) a > b ? a : b end
  # INF = Float::INFINITY
  # def chmin(a, b) a < b ? a : b end
DEBUG = true

t = gets.to_i

a = []
s = []
t.times do
  gets.to_i
  a << gets.split.map(&:to_i)
  s << gets.chomp.split("").map(&:to_i)
end

t.times do |i|
  x = 0
  ary = a[i]
  s[i].each_with_index do |player,index|
    if DEBUG
      puts "---"
      puts "x: #{x}"
      puts "player: #{player}"
      puts "ary[index]: #{ary[index]}"
    end
    if player == 0
      if x ^ ary[index] == 0 || ary.slice(index+1, ary.length).include?(x ^ ary[index])
        x = x ^ ary[index]
      end
    elsif player == 1
      if x ^ ary[index] != 0
        unless ary.slice(index+1, ary.length).include?(x ^ ary[index])
          x = x ^ ary[index]
        end
      end
    end
    if DEBUG
      puts "change x: #{x}"
    end
  end

  puts x == 0 ? 0 : 1 
end